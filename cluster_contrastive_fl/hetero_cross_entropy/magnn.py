# The code is modified based on the code from the paper 
...
class GraphClassifier(th.nn.Module):
    """
    Description
    -----------
    ...
    def __init__(self):
        ...
        self.magnn = MAGNN(in_feats = {'I': 300, 'S':300, 'A':512},
                                        ...
                                        edge_type_list=['S-I', 'I-S'], dropout_rate = 0.2, encoder_type = 'Average', activation = F.elu)

        self.conv1 = GraphConv(300, 128, allow_zero_in_degree=True)

        self.conv2 = torch.nn.Linear(128, 64) # use the name to reduce code workload
        self.conv3 = torch.nn.Linear(64, 2)
        self.dropout = 0.5


    def forward(self, graph:DGLGraph, node_feat:Optional[Tensor]=None, edge_feat:Optional[Tensor]=None):
        ...
        h = self.conv1(new_homo_graph, new_homo_graph.ndata['feat'])
        h = F.relu(h)
        h = torch.mean(h, 0, True)

        logits = F.relu(self.conv2(h))
...
#         self.conv1 = GraphConv(300, 256, allow_zero_in_degree=True)
#         self.conv2 = GraphConv(256, 128, allow_zero_in_degree=True)
#         self.conv3 = GraphConv(128, 100, allow_zero_in_degree=True)
...
#         h = self.conv1(new_homo_graph, new_homo_graph.ndata['feat'])
#         h = F.relu(h)
#         h = self.conv2(new_homo_graph, h)
#         h = F.relu(h)
#         h = self.conv3(new_homo_graph, h)
#         new_homo_graph.ndata['hn'] = h
#         return dgl.mean_nodes(new_homo_graph, 'hn')
