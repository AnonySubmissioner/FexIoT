# we use "dive-into-graphs 0.1.2", and we add new function MCBS in this file. Replace original subgraphx.py with this new one.
...
from .shapley import GnnNetsGC2valueFunc, GnnNetsNC2valueFunc, \
    gnn_score, mc_shapley, l_shapley, mc_l_shapley, NC_mc_l_shapley, sparsity, mc_l_SHAP
...
def reward_func(reward_method, value_func, node_idx=None,
    ...
    elif reward_method.lower() == 'mc_l_shap':
        return partial(mc_l_SHAP,
...
class MCBS(object):
    r"""
    Monte Carlo Beam Search Method
    Args:
    ...
    def mcbs_rollout(self, tree_node):
        ...
        if len(tree_node.children) == 0:
            ...
            half = int(len(all_nodes)/2)
            if half > 1:
                all_nodes = all_nodes[:half]
        ...
        v = self.mcbs_rollout(selected_node)
        selected_node.W += v
        selected_node.N += 1
        return v

    def mcbs(self, verbose=True):
        if verbose:
            print(f"The nodes in graph are {self.graph.number_of_nodes()}")
        for rollout_idx in range(self.n_rollout):
            self.mcbs_rollout(self.root)
            if verbose:
