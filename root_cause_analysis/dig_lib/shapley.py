# we use "dive-into-graphs 0.1.2", and we add new function SHAP in this file. Replace original subgraphx.py with this new one.
...
def KernelSHAP(data: Data, exclude_mask: np.array, include_mask: np.array,
    ...
    for epoch in range(100):
        for exclude_data, include_data in dataloader:
            ...
            tmp_exclude_values = value_func(exclude_data)

            exclude_values = linear_regression(tmp_exclude_values)

            tmp_graph = to_networkx(include_data)
            M = tmp_graph.number_of_nodes()
            K = np.sum(exclude_mask)
            coeff = (M-1)/(comb(M, K)*K*(M-K))
            loss = criterion(exclude_values, include_values) * coeff
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    mc_l_SHAP_value = float(torch.sum(linear_regression.linear.weight.data))
    return mc_l_SHAP_value
...
def mc_l_SHAP(coalition: list, data: Data, local_radius: int,
                 value_func: str, subgraph_building_method='zero_filling',
                 sample_num=1000) -> float:
    """ monte carlo sampling approximation of the l_SHAP value """
    ...
    mc_l_SHAP_value = \
        KernelSHAP(data, exclude_mask, include_mask, value_func, subgraph_build_func)
...
def mc_l_shapley(coalition: list, data: Data, local_radius: int,
    ...
    print("mc_l_shapley_value", mc_l_shapley_value)
...
def sparsity(coalition: list, data: Data, subgraph_building_method='zero_filling'):
    if subgraph_building_method == 'zero_filling':
        ...
        edge_mask = (node_mask[row] == 1) & (node_mask[col] == 1)
        return 1.0 - np.true_divide(edge_mask.sum() , edge_mask.shape[0])
